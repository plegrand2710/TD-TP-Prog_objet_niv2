...
public class ControleurDessin extends MouseAdapter {
    ...
    public void mousePressed(MouseEvent e) {
        ...
        if (outil == OutilDessin.GOMME) {
            couleur = vue.getZoneDessin().getBackground();
        } else {
            couleur = vue.getCouleurTrait();
        }
    ...
    public void mouseDragged(MouseEvent e) {
        if (formeEnCours != null) {
            if (formeEnCours.getOutil() == OutilDessin.PINCEAUX || formeEnCours.getOutil() == OutilDessin.GOMME) {
                formeEnCours.ajouterPoint(e.getPoint());
            } else if (formeEnCours.getOutil() == OutilDessin.LIGNE || formeEnCours.getOutil() == OutilDessin.RECTANGLE || formeEnCours.getOutil() == OutilDessin.CERCLE) {
    ...
    public void mouseReleased(MouseEvent e) {
        if (formeEnCours != null) {
            if (formeEnCours.getOutil() == OutilDessin.LIGNE || formeEnCours.getOutil() == OutilDessin.RECTANGLE || formeEnCours.getOutil() == OutilDessin.CERCLE) {
                if (formeEnCours.getPoints().size() == 1) {
                    formeEnCours.ajouterPoint(e.getPoint());
                } else {
                    formeEnCours.getPoints().set(1, e.getPoint());
